...
func NewMemoryHandler(logger *logrus.Logger) *MemoryHandler {
	...
	pressureConfig := &memory.PressureHandlerConfig{
		...
		Thresholds: &memory.PressureThresholds{
			LowPressure: &memory.Threshold{
				HeapUtilization:   40.0,
				GoroutineCount:    1000,
				AllocationRate:    10 * 1024 * 1024,
				GCFrequency:       0.1,
				SustainedDuration: time.Second * 30,
			},
			MediumPressure: &memory.Threshold{
				HeapUtilization:   70.0,
				GoroutineCount:    2000,
				AllocationRate:    25 * 1024 * 1024,
				GCFrequency:       0.3,
				SustainedDuration: time.Second * 15,
			},
			HighPressure: &memory.Threshold{
			...
			CriticalPressure: &memory.Threshold{
				HeapUtilization:   95.0,
				GoroutineCount:    10000,
				AllocationRate:    100 * 1024 * 1024,
				GCFrequency:       1.0,
	...
	preallocationConfig := &memory.PreallocationConfig{
		...
		MinUsageThreshold:  0.3,
		MaxPreallocationMB: 50,
		AdaptiveScaling:    true,
		PredictiveEnabled:  true,
		AggressiveMode:     false,
		EnabledStrategies:  []string{"buffer_pool", "json_objects"},
	}
...
func (mh *MemoryHandler) GetMemoryStatus(c *gin.Context) {
	...
	pressure := mh.pressureHandler.GetStatus()
	...
	status := gin.H{
		...
		"pressure": gin.H{
			"level":       pressure.CurrentLevel.String(),
			"event_count": pressure.EventCount,
			"last_event":  pressure.LastEvent,
		},
		"pools": gin.H{
			"count": len(poolStats),
			"stats": poolStats,
...
func (mh *MemoryHandler) OptimizeMemory(c *gin.Context) {
	...
	for _, component := range request.Components {
		...
		case "pools":
			// Pool optimization is handled automatically
			results[component] = gin.H{
		...
		case "preallocation":
			// Use OptimizePools method
			err := mh.preallocationManager.OptimizePools()
			if err != nil {
				results[component] = gin.H{
					"status":     "error",
					"error":      err.Error(),
...
func (mh *MemoryHandler) AnalyzeUsagePatterns(c *gin.Context) {
	...
	patterns, err := mh.preallocationManager.AnalyzeUsagePatterns()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": "error",
			"error":  err.Error(),
...
func (mh *MemoryHandler) StartOptimization(c *gin.Context) {
	if err := mh.optimizationEngine.Start(c.Request.Context()); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status": "error",
			"error":  err.Error(),
		})
...
func (mh *MemoryHandler) GetOptimizationHistory(c *gin.Context) {
	...
	history := mh.optimizationEngine.GetOptimizationHistory(limit)
...
func (mh *MemoryHandler) calculateOverallHealthScore(stats *memory.MemoryStats, pressure *memory.PressureStatus) float64 {
	score := 100.0
	...
	// Deduct for high memory pressure based on current level
